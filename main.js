...
const copy_code = (opj) => {
    console.log(opj.parentElement.childNodes[2].childNodes[1].textContent);
    ...
    if(value.IMAGES.length > 0){
        value.IMAGES.forEach(image =>{
            ...
            if(image.DESCRIBE !== null && image.DESCRIBE !== ""){
                template += `<p class="content-body--describe">${image.DESCRIBE}</p>`;
            }
        })
    }
        ...
        if(itemDetail.CODE !== null && itemDetail.CODE !== "" && itemDetail.LANGUAGE !== null && itemDetail.LANGUAGE !== ""){
            ...
            if(itemDetail.IMAGES !== null && itemDetail.IMAGES.length > 0){
                itemDetail.IMAGES.forEach(image =>{
                    ...
                    if(image.DESCRIBE){
                        template += `<p class="content-body-detail__code--describe">${image.DESCRIBE}</p>`
                    }
                })
            }
